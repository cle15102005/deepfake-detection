...
def preprocess_image(img, label, IMAGE_SIZE):
    """
    Resize and apply EfficientNet-specific preprocessing.

    EfficientNet preprocessing:
    - Resizes to 224x224
    - Normalizes to range expected by ImageNet-pretrained model
    - Uses tf.keras.applications.efficientnet.preprocess_input()
    """
    # Set shape explicitly for resize operation
    img.set_shape([None, None, 3])

    # Resize to EfficientNet input size
    img = tf.image.resize(img, IMAGE_SIZE)

    # Apply EfficientNet-specific preprocessing: normalizes pixel values to the range the model expects
...
def make_data(train_df, val_df, test_df, BATCH_SIZE):
    ...
    train_dataset = (
        ...
        .map(preprocess_image, num_parallel_calls=AUTOTUNE) # Preprocess for EfficientNet
...
def verify_pipeline(train_dataset, plot_dir, file_dir):
    ...
    # Check pixel value range (EfficientNet preprocessing changes this)
